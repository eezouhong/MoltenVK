...
void MVKQueryPool::applyMetal4End(uint32_t query) {
	lock_guard<mutex> lock(_availabilityLock);
	_availability[query] = DeviceAvailable;
}

void MVKQueryPool::finishMetal4Query(uint32_t query) {
	finishQueries(MVKArrayRef<const uint32_t>(&query, 1));
}

bool MVKQueryPool::canEndMetal4Query() {
	lock_guard<mutex> lock(_deferredCopiesLock);
	return _deferredCopies.empty();
}
