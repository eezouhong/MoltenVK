...
# Exact-SHA Phase 1C gate after the type-correct mixed-bitset repair: run the
# same Vulkan binary with the command backend disabled and enabled; only
# MoltenVK's internal backend selection may differ.
